// Based on your One-Hot Address: Word Bit 3 -> 0x00400020
#define GPIO_REG *((volatile unsigned int*)0x00400020)

void _start(void) {
    // 1. Write a distinct pattern to the IP
    // You should see this value appear in GTKWave on the GPIO_reg signal
    GPIO_REG = 0xDEADE123;

    // 2. Read it back into a local variable
    // This will trigger the 'assign IP_rdata' logic in your Verilog
